<?php
	...
	//		$a = [];
            while ( strlen($attendancedata) > 40 ) {
				...
				//$a[] = intval( str_replace("\0", '', hex2bin( substr($u[1], 6, 8) ) ) );
	//			$s = str_replace("\0", '', hex2bin( substr($u[1], 6, 8) ) );
	...
	//			$a[] = $s;
                ...
                $uid 	   = $u1+($u2*256);
    //          $id 	   = intval( str_replace("\0", '', hex2bin( substr($u[1], 6, 8) ) ) ); // has problem
	//			$id 	   = str_replace("\0", '', hex2bin( substr($u[1], 6, 8) ) ); 
				$id 	   = hex2bin( substr($u[1], 6, 8) ) ; // as revised by alvin
